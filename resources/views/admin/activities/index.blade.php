...
<div class="space-y-8" x-data="{ addModal: false, editModal: false, activeActivity: {} }">
    
    <div class="flex items-center justify-between">
        <div>
            <h2 class="font-display text-2xl font-bold text-slate-900">Program & Kegiatan Harian</h2>
            <p class="text-sm text-slate-500">Ubah judul, deskripsi, warna, dan upload foto kegiatan anak di TPA Robbani.</p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl space-y-4">
            ...
            <form action="{{ route('admin.activities.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="desc" rows="3" required placeholder="Penjelasan singkat kegiatan..." class="w-full px-4 py-3 rounded-xl border border-slate-300 text-sm"></textarea>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Upload Foto Kegiatan (Opsional)</label>
                    <input type="file" name="image" accept="image/*" class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-pink-50 file:text-pink-600">
                </div>
                <div>
    ...
    <div x-show="editModal" x-transition class="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4" style="display: none;">
        <div class="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl space-y-4">
            <h3 class="font-display text-xl font-bold text-slate-900">Edit Kegiatan Harian</h3>
            
            <form :action="'{{ url('admin/activities') }}/' + activeActivity.id" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Judul Kegiatan *</label>
                    <input type="text" name="title" x-model="activeActivity.title" required class="w-full px-4 py-3 rounded-xl border border-slate-300 text-sm">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Deskripsi Kegiatan *</label>
                    <textarea name="desc" rows="3" x-model="activeActivity.desc" required class="w-full px-4 py-3 rounded-xl border border-slate-300 text-sm"></textarea>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Ganti Foto Kegiatan (Upload Baru)</label>
                    <input type="file" name="image" accept="image/*" class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-pink-50 file:text-pink-600">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Gradien Warna Card</label>
                    <select name="color" x-model="activeActivity.color" class="w-full px-4 py-3 rounded-xl border border-slate-300 text-sm bg-white">
                        <option value="from-pink-500 to-rose-500">Pink / Rose</option>
                        <option value="from-purple-500 to-indigo-500">Purple / Indigo</option>
                        <option value="from-cyan-500 to-blue-500">Cyan / Blue</option>
                        <option value="from-amber-400 to-orange-500">Amber / Orange</option>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit" class="px-6 py-2.5 rounded-xl font-bold text-xs text-white bg-pink-600 hover:bg-pink-700 shadow-md">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-3xl overflow-hidden border border-slate-200 shadow-xs flex flex-col justify-between">
            <div>
                @if($act->image)
                <img src="{{ asset($act->image) }}" alt="{{ $act->title }}" class="w-full h-44 object-cover">
                @endif
                <div class="p-6">
                    <div class="flex items-center justify-between mb-3">
                        <span class="w-8 h-8 rounded-lg bg-gradient-to-r {{ $act->color }} text-white font-bold flex items-center justify-center text-xs">
                            #{{ $act->order }}
                        </span>
                    </div>
            ...
            <div class="px-6 pb-6 pt-2 flex items-center justify-between border-t border-slate-100">
                <button @click="activeActivity = {{ json_encode($act) }}; editModal = true" class="text-xs font-bold text-pink-600 hover:underline">
